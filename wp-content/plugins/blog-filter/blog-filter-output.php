	...
	<div class="blog_filter_main" >
			...
			if ( $terms && ! is_wp_error( $terms ) ) :
				...
				<div class="text-center">
					...
					if ( $blog_filter_all == 'yes' ) {
						?>
						<li id="all" class="snip0047 active" data-filter="all"><span style="pointer-events: none;"><?php echo esc_html(__( $blog_all_text, 'blog-filter' )); ?></span><i class="fa fa-check" style="pointer-events: none;"></i></li>
						<?php
					} foreach ( $terms as $term ) {
						?>
						<li class="snip0047" value="<?php echo esc_attr( $term->term_id ); ?>" data-filter="<?php echo esc_attr( $term->term_id ); ?>"><span style="pointer-events: none;"><?php echo esc_html(__( $term->name, 'blog-filter' )); ?></span><i class="fa fa-check" style="pointer-events: none;"></i></li>
						<?php
					} ?>
					</ul>
				</div>
		...
		<div id="bf_gallery_1" class="filtr-container filters-div " style="opacity:0;">
			...
			while ( $custom_query->have_posts() ) :
				...
				?>
				<div id="bf_<?php echo get_the_ID(); ?>" data-category="<?php echo esc_attr( $keys ); ?>" data-sort="<?php echo esc_attr( $filter_value->name ); ?>" class=" pfg_theme_1 filtr-item filtr_item_1 single_one col-lg-4 col-md-4 col-sm-6 col-xs-12">
					<div class="bf_thumb_box_1 hvr-shadow-radial">
						<div class="bf_title_box_1">
							<?php
							if ( $blog_title == 'yes' ) {
								?>
								<h3 class="bf_title_1"><?php echo esc_html_e( ucwords( the_title() ) ); ?></h3>
								<?php
							}
							?>
						</div>
						...
						<div class="bf_title_box_2">
							...
							if ( $blog_date == 'yes' ) {
								...
								?>
								<div class="metaInfo">
									<span><i class="fa fa-calendar"></i> <a href="<?php echo esc_url(get_day_link( $year, $month, $day )); ?>"><?php the_time( 'j F, Y' ); ?></a> </span>
								</div>
								<?php
							} if ( $blog_desc == 'yes' ) {
								?>
								<div class="bf_desc_1">
									...
									echo esc_html(ucfirst( stripcslashes( substr( get_the_excerpt(), 0, $blog_desc_words ) ) . '...' )); ?>
								</div>
								<?php
