...
?>
<div id="awp-flickr-album-<?php echo esc_attr( $flickr_gallery_id ); ?>" class="awp-flickr-album-<?php echo esc_attr( $flickr_gallery_id ); ?>">	
	<?php
	foreach ( $res as $sp ) {
		// if($album_title == "true") {
			$album_title_fetch = esc_html( $sp['title'] );
		// }
		...
		if ( $apply_light_box == 'true' ) {
			...
			if ( $image_type == 'image' && $thumbnail_url ) {
				?>
				<div class="img-responsive text-center single-album-<?php echo esc_attr( $flickr_gallery_id ); ?> <?php echo esc_attr( $col_desktops ); ?>">
					<img class="img-responsive img-thumbnail photo loading" src="<?php echo esc_url( $thumbnail_url ); ?>" alt="<?php echo esc_html( $album_title_fetch ); ?>" width="auto" height="auto">
				</div> 
				<?php
			}
...
<style>
.awp-flickr-album-<?php echo esc_attr( $flickr_gallery_id ); ?> a {
	text-decoration: none !important;
	box-shadow: 0px 0px 0 0 currentColor !important;
	padding-bottom: 10px;
}
.awp-flickr-album-<?php echo esc_attr( $flickr_gallery_id ); ?> div {
	padding-bottom: 10px;
}
</style>
