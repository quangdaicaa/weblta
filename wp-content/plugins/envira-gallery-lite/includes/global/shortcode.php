...
class Envira_Gallery_Shortcode {
	...
	public function generate_gallery_item_markup( $gallery, $data, $item, $id, $i ) {
		...
		if ( $create_link ) {
			$output .= '<a href="' . esc_url( $item['link'] ) . '" class="' . ( ( isset( $item['link_new_window'] ) && 1 === $item['link_new_window'] ) ? '' : 'envira-gallery-' . sanitize_html_class( $data['id'] ) . ' ' ) . 'envira-gallery-link" ' . $html5_attribute . '="enviragallery' . sanitize_html_class( $data['id'] ) . '" title="' . $title . '" data-envira-caption="' . $caption . '" data-envira-retina="' . ( isset( $item['lightbox_retina_image'] ) ? $item['lightbox_retina_image'] : '' ) . '" data-thumbnail="' . esc_url( $item['thumb'] ) . '"' . ( ( isset( $item['link_new_window'] ) && 1 === $item['link_new_window'] ) ? ' target="_blank"' : '' ) . ' ' . apply_filters( 'envira_gallery_output_link_attr', '', $id, $item, $data, $i ) . ' itemprop="contentUrl">';
		}
