...
jQuery(document).ready(function($) {
    ...
    switch( action ) {
        case 'activate' : $( '.mt-activate-demo-import-plugin' ).on( 'click', function() {
                var _this = $( this );
                edigital_do_plugin( 'activate_demo_importer_plugin', _this );
            });
            break;
        case 'install' : $( '.mt-install-demo-import-plugin' ).on( 'click', function() {
                var _this = $( this );
                edigital_do_plugin( 'install_demo_importer_plugin', _this );
            });
            break;
    }
    
    edigital_do_plugin = function ( ajax_action, _this ) {
        $.ajax({
            method : "POST",
            url : WpAjaxurl,
            data : ({
